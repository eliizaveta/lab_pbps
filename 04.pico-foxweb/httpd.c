...
void serve_forever(const char *PORT) {
  ...
  char log[1000];
  sprintf(log, "%s%s%s%s", "Server start running using port:", PORT, ", resource catalog:", PUBLIC_DIR);
...
void respond(int slot) {
  ...
  else // message received
  {
    ...
    sprintf(log, "%s - - [%s] \"%s %s %s\" - -", ip, date, method, uri, prot);
    ...
    //Журнализация в собственный текстовой журнал HTTP-запросов
    sprintf(log, "%s %d %d [%s] \"%s %s %s\"", ip, clientfd, clientfd, date, method, uri, prot);    

    // call router
    route(log);
    sprintf(log, "%s %d", log, payload_size);
    myLog = fopen("var/log/myLogs/myLog.log", "a");
    fprintf(myLog, "%s\n", log);
    fclose(myLog);
